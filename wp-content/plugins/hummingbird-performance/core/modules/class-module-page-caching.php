...
class WP_Hummingbird_Module_Page_Caching extends WP_Hummingbird_Module {
	...
	public function get_settings() {
		...
		if ( file_exists( $config_file ) ) {
			...
			self::log( 'Config file not found at: ' . $config_file, 'page-caching' );
			$settings = array(
	...
	private static function get_file_cache_path( $request_uri ) {
		...
		self::log( 'Caching to file: ' . $wphb_cache_file, 'page-caching' );
	}
	 ...
	 * @used-by WP_Hummingbird_Module_Page_Caching::clear_cache()
	...
	private static function get_cookies() {
		...
		if ( ! empty( $cookie_value ) ) {
			self::log( 'Cookie cached: ' . $cookie_value, 'page-caching' );
			return $cookie_value;
		}
		...
		foreach ( (array) $_COOKIE as $key => $value ) { // Input var ok.
			...
			if ( preg_match( '/^wp-postpass_|^comment_author_|^wordpress_logged_in_/', $key ) ) {
				self::log( 'Found cookie: ' . $key, 'page-caching' );
		...
		if ( ! empty( $cookie_value ) ) {
			...
			self::log( 'Cookie hashed value: ' . $cookie_value, 'page-caching' );
		}
	...
	public function save_settings( $settings ) {
		...
		self::log( 'Writing configuration to: ' . $config_file, 'page-caching' );
		$this->write_file( $config_file, json_encode( $settings ) );
	...
	public function disable() {
		...
		if ( $wphb_fs->purge( 'cache' ) ) {
			self::log( 'Page cache deactivation: successfully purged cache folder.', 'page-caching' );
		} else {
			self::log( 'Page cache deactivation: error purging cache folder.', 'page-caching' );
		}
		...
		if ( false !== strpos( file_get_contents( $adv_cache_file ), 'WPHB_ADVANCED_CACHE' ) ) {
			...
			self::log( 'Page cache deactivation: advanced-cache.php file removed.', 'page-caching' );
		}
	}
	...
	private function write_wp_config( $uninstall = false ) {
		...
		if ( ! file_exists( $config_file ) ) {
			self::log( 'Failed to locate wp-config.php file.', 'page-caching' );
			return false;
		}

		$fp = fopen( $config_file, 'r+' );
		if ( ! $fp ) {
			self::log( 'Failed to open wp-config.php for writing.', 'page-caching' );
			return false;
		}
		...
		foreach ( $lines as $line ) {
			if ( preg_match( "/define\(\s*\'WP_CACHE/i", $line ) ) {
				$found_code = true;
				if ( ! $uninstall ) {
					self::log( "Added define('WP_CACHE', true) to wp-config.php file.", 'page-caching' );
					$new_file[] = "define('WP_CACHE', true); // Added by WP Hummingbird";
				} else {
					self::log( "Removed define('WP_CACHE', true) from wp-config.php file.", 'page-caching' );
				}
	...
	private static function should_cache_request( $request_uri ) {
		...
		if ( ( defined( 'DOING_CRON' ) && DOING_CRON ) || ( defined( 'DOING_AJAX' ) && DOING_AJAX ) ) {
			self::log( 'Page not cached because of active cron or ajax request.', 'page-caching' );
			return false;
		} elseif ( is_admin() ) {
			self::log( 'Do not cache admin pages.', 'page-caching' );
			return false;
		} elseif ( self::logged_in_user() && ! $wphb_cache_config->cache_logged_in ) {
			self::log( 'Do not cache pages for logged in users.', 'page-caching' );
			return false;
		} elseif ( isset( $_SERVER['REQUEST_METHOD'] ) && 'GET' !== $_SERVER['REQUEST_METHOD'] ) { // Input var okay.
			self::log( 'Skipping page. Used ' . $_SERVER['REQUEST_METHOD'] . ' method. Only GET allowed.', 'page-caching' );
			return false;
		} elseif ( isset( $_GET['preview'] ) ) { // Input var okay.
			self::log( 'Do not cache preview post pages.', 'page-caching' );
			return false;
		} elseif ( false === empty( $_GET ) && ! $wphb_cache_config->cache_with_get_params ) { // Input var ok.
			self::log( 'Skipping page with GET params.', 'page-caching' );
			return false;
			...
			self::log( 'Do not cache wp-admin pages.', 'page-caching' );
			return false;
		} elseif ( self::skip_url( $request_uri ) ) {
			self::log( 'Do not cache page. URL exclusion rule match: ' . $request_uri, 'page-caching' );
			return false;
		} elseif ( self::skip_user_agent() ) {
			self::log( 'Do not cache page. User-Agent is empty or excluded in settings.', 'page-caching' );
			return false;
		} elseif ( ! isset( $_SERVER['HTTP_HOST'] ) ) { // Input var ok.
			self::log( 'Page can not be cached, no HTTP_HOST set.', 'page-caching' );
			return false;
		} // End if().

		// TODO Check for object cache?
		self::log( 'Request passed should_cache_request check. Ready to cache.', 'page-caching' );
	...
	public function cache_request( $buffer ) {
		...
		if ( empty( $buffer ) ) {
			$cache_page = false;
			self::log( 'Empty buffer. Exiting.', 'page-caching' );
		}

		if ( defined( 'DONOTCACHEPAGE' ) && DONOTCACHEPAGE ) {
			$cache_page = false;
			self::log( 'Page not cached because DONOTCACHEPAGE is defined.', 'page-caching' );
		} elseif ( is_404() ) {
			$cache_page = false;
			self::log( 'Do not cache 404 pages.', 'page-caching' );
		} elseif ( self::skip_page_type() ) {
			$cache_page = false;
			self::log( 'Do not cache page. Skipped in settings.', 'page-caching' );
		} elseif ( ! preg_match( '/(<\/html>|<\/rss>|<\/feed>|<\/urlset|<\?xml)/i', $buffer ) ) {
			$cache_page = false;
			self::log( 'HTML corrupt. Page not cached.', 'page-caching' );
		}

		if ( ! $cache_page ) {
			self::log( 'Page not cached. Sending buffer to user.', 'page-caching' );
			return $buffer;
		}
		...
		if ( $wphb_cache_file ) {
			self::log( 'Saving page to cache file: ' . $wphb_cache_file, 'page-caching' );
	...
	public function clear_cache( $directory = 'cache' ) {
		...
		// Purge cache directory.
		if ( 'cache' === $directory ) {
			self::log( 'Cache direcotry purged', 'page-caching' );
	...
	private function purge_post_cache( $post_id ) {
		...
		self::log( 'Cache for has been purged for post id: ' . $post_id, 'page-caching' );
		...
		foreach ( $meta_array as $meta_name => $meta_key ) {
			...
			foreach ( $metas as $meta ) {
				...
				self::log( "Cache has been purged for {$meta_name}: {$meta->name}", 'page-caching' );
			}
		}
		...
		if ( $author_link ) {
			...
			self::log( "Cache has been purged for author page: $author_link", 'page-caching' );
		}
	}
	...
	public static function serve_cache() {
		...
		if ( file_exists( $wphb_cache_file ) ) {
			self::log( 'Cached file found. Serving to user.', 'page-caching' );
	...
	public function init_caching() {
		...
		if ( file_exists( $wphb_cache_file ) ) {
			...
			self::log( 'Cached file not found. Passing to ob_start.', 'page-caching' );
